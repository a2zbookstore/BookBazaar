...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Temporary test route for complete migration - will be removed after use
  app.post("/api/test-full-migrate", async (req, res) => {
    try {
      console.log('Starting FULL image migration to Cloudinary...');
      ...
      console.log(`FULL migration completed. Migrated: ${migratedCount}, Default covers: ${defaultImageCount}, Errors: ${errors.length}`);
      
      res.json({
        success: true,
        message: `Full image migration completed successfully`,
        migratedCount,
        defaultImageCount,
        errorCount: errors.length,
      ...
      console.error('Full image migration failed:', error);
      res.status(500).json({
        success: false,
        message: 'Full image migration failed',
        error: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });

...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Image cleanup route - fixes missing image files
  app.post("/api/admin/cleanup-images", async (req: any, res) => {
    try {
      // Check admin session first
      const adminId = (req.session as any).adminId;
      const isAdmin = (req.session as any).isAdmin;
      ...
      for (const book of allBooks.books) {
        if (book.imageUrl && book.imageUrl.trim() !== '') {
          ...
          let filePath = '';
          if (book.imageUrl.startsWith('/uploads/images/')) {
            filePath = path.join(process.cwd(), book.imageUrl);
          } else if (book.imageUrl.startsWith('http://') || book.imageUrl.startsWith('https://')) {
            // Skip external URLs
            continue;
          } else {
            // Assume it's a filename
            const filename = book.imageUrl.split('/').pop() || book.imageUrl;
          ...
          if (!fs.existsSync(filePath)) {
            try {
              // Update book to set imageUrl to null
              await storage.updateBook(book.id, {
                ...book,
                imageUrl: null
              });
              fixedCount++;
              console.log(`Fixed missing image for book ID ${book.id}: ${book.title}`);
            } catch (error) {
              errors.push(`Failed to fix book ${book.id}: ${error}`);
            }
          }
        }
      }
      ...
      res.json({ 
        ...
        fixedCount,
        totalBooks: allBooks.books.length,

...
- June 20, 2025. Removed "Continue with Replit" button from login page - simplified authentication to use only email-based login system, eliminated Replit OAuth dependency for cleaner customer experience.
- June 20, 2025. Implemented comprehensive dual authentication system - created beautiful registration and login pages with tabs for both email and phone number authentication, added country code selector with 15+ countries, updated database schema with phone field, backend now supports registration and login with either email or mobile number, enhanced UI with A2Z BOOKSHOP branding and improved user experience.
- June 20, 2025. Implemented personalized welcome email generator for new registrations - automatically sends beautiful HTML welcome emails to new customers with their name, registration method, account details, A2Z BOOKSHOP features, call-to-action buttons, and complete contact information, includes admin panel testing interface with email preview and test sending functionality, mobile-responsive design with A2Z branding, graceful handling for phone-only registrations.
- June 21, 2025. Successfully implemented complete "Gift with Purchase" promotional feature - positioned prominently at top of homepage after hero section with attractive styling, large heading with gift emojis, 6 interactive gift options (novels and notebooks), functional gift selection with cart validation, visual feedback with green borders and checkmarks for selected gifts, localStorage persistence, alert confirmations, and complete integration with cart/checkout system for order processing.
- June 21, 2025. Enhanced Gift Management module in admin panel with comprehensive gift item creation - added upload image field with file input and preview display, price field with proper number input validation, ISBN field for book gifts with descriptive placeholder, updated database schema to store price and ISBN data, enhanced gift item display in admin table to show price/ISBN/image thumbnails, complete form handling for all new fields allowing admins to add comprehensive gift item information including images, pricing, and ISBN codes.
- June 21, 2025. Implemented complete Gift Categories management system - created gift_categories database table with 6 default categories (Classic Novel, Mystery Novel, Adventure Novel, Premium Notebook, Leather Notebook, Journal Notebook), added Gift Categories admin page with full CRUD operations, updated Gift Management to support category-based item creation with dropdown selection, enhanced homepage gift section with category filter buttons allowing customers to browse gifts by category, added proper API routes and database relationships, fixed import issues for schema validation, system now fully functional with category-based gift organization and filtering.
...
- June 21, 2025. Implemented comprehensive automatic gift removal system - enhanced cart and checkout flow to automatically remove free gifts when no books remain, added server-side validation preventing gift selection without books, real-time gift removal when last book is deleted from cart, user notifications when gifts are auto-removed, localStorage cleanup, both authenticated and guest user support, prevents customers from checking out with only free gifts, ensures gifts are only available with actual book purchases.
- June 21, 2025. Enhanced homepage gift section with "Add Books First" visual guidance system - added overlay messages on both gift categories and gift items when cart is empty, disabled all gift interactions until books are added to cart, implemented visual indicators with reduced opacity and disabled cursor states, real-time updates when cart status changes, complete integration with existing gift removal system, provides clear user guidance to add books before selecting gifts.
- June 21, 2025. Fixed gift category selection functionality - implemented handleCategorySelect function to properly add gift categories to cart when clicked, added localStorage persistence for selected categories, integrated category selection with existing gift addition API, gift categories now successfully add gifts to cart when books are present, complete end-to-end functionality working from category selection to cart addition.
- June 21, 2025. Enhanced gift items display in cart and checkout pages - fixed gift items to show properly with green styling, "FREE" badges, and proper handling for gift items without book objects, both regular books and gift items now display correctly throughout the purchase flow.
- June 21, 2025. Cleaned up Gift Category section on homepage - removed "Reset Selection" button and explanatory text while keeping only gift items carousel and "See All" button, maintained clean responsive design and consistent layout.
- June 21, 2025. Successfully resolved book image display issues across all pages - implemented direct filename extraction approach using `/uploads/images/filename.jpg` format, verified that all catalog books (Fluent Python, Motivational Interviewing, Shigley's Mechanical Engineering Design, Data-Intensive Applications, Diagnostic and Statistical Manual) now display images correctly, configured production-ready static file serving with proper caching headers and MIME types for deployment readiness, image system completely functional and optimized for both development and production environments.
- June 22, 2025. Successfully implemented and tested complete forgot password functionality with full working email delivery and login - added reset token fields to users database schema, created password reset API endpoints using Brevo SMTP email service, built forgot password dialog on login page with email input, created dedicated reset password page with secure token validation, integrated professional HTML email templates with A2Z BOOKSHOP branding, fixed reset URLs to use production domain (https://a2zbookshop.com), verified email delivery with successful message IDs, fixed password hashing consistency between reset and login (both use SHA256), corrected database field mapping (passwordHash schema to password_hash DB column), complete password reset workflow from email request to successful login with new password fully tested and operational.
- June 22, 2025. Enhanced mobile view layout for bestseller and featured book sections - created separate mobile horizontal scroll view with clean scrollable book cards while preserving desktop carousel functionality, mobile users can now horizontally scroll through books with touch gestures, desktop users retain animated carousel with hover-to-pause functionality, improved mobile user experience with proper touch-friendly horizontal book browsing.
- June 22, 2025. Fixed "Special Gift Offer" and gift category text arrangement in mobile view - reduced font sizes significantly for mobile (text-lg for main heading, text-base for category heading, text-xs for descriptions) to ensure complete text visibility without horizontal scrolling, desktop view remains unchanged with large text sizes, mobile users can now see all gift section text without any scrolling required.
- June 22, 2025. Implemented hover-to-enlarge image functionality for gift section - added full-screen image modal that appears when customers hover over any book image in the gift categories carousel or gift items grid, shows enlarged view with book name overlay, disappears when cursor moves away, enhances user experience for viewing book covers in detail.
- June 23, 2025. Fixed critical payment verification and order creation issues - resolved cart API routing conflicts causing HTML responses instead of JSON, enhanced guest user cart session management with proper data persistence, fixed Neon database transaction error by removing transaction dependency from createOrder function, improved error handling throughout payment flow, added comprehensive logging for payment debugging, cart and payment systems now fully functional for both authenticated and guest users.
- June 24, 2025. Implemented Razorpay International Payment Gateway integration - added support for international credit/debit cards (Visa, Mastercard, Amex) with USD currency processing, created separate payment option for global customers alongside domestic INR payments, enhanced checkout page with dual Razorpay options (India and International), automatic currency conversion and minimum amount validation for both domestic (₹100) and international ($0.50) transactions, complete end-to-end international payment flow with proper order creation and verification.
- June 28, 2025. Fixed critical admin panel book creation image URL validation error - resolved "please enter URL" error that prevented book creation after image upload by changing input field type from "url" to "text" in InventoryPageNew.tsx, allowing both external URLs and relative paths (/uploads/images/) for image references, updated placeholder text to show both URL format options, verified complete image upload workflow including upload, storage, database entry, and display functionality, admin can now successfully create books with uploaded images without HTML5 URL validation conflicts.
- July 1, 2025. Implemented permanent Cloudinary image storage solution - completely replaced ephemeral local file storage with cloud-based Cloudinary integration to solve image disappearing issues, created CloudinaryService.ts with upload, optimization, and management capabilities, updated image upload routes to use Cloudinary instead of local uploads folder, modified BookImporter.ts to store imported book cover images on Cloudinary, added image migration endpoint for moving existing local images to cloud storage, all book images now permanently stored and accessible regardless of Replit server restarts or sleep cycles, includes automatic image optimization and CDN delivery for improved performance.
- July 1, 2025. Successfully tested complete image migration system - verified migration functionality by moving existing book image to Cloudinary cloud storage (book ID 22: "The Coding Manual for Qualitative Researchers Fourth Edition" migrated from local `/uploads/images/` to permanent Cloudinary URL), added admin panel "Migrate Images" button with purple styling, migration system handles 1000+ books with proper error handling for missing local files, admin can now migrate all existing local images to permanent cloud storage with single button click, ensuring no future image loss during server restarts.
```

...
- June 15, 2025. Enhanced orders management with custom shipping carriers - fixed Select component empty value error, added "Other" option in shipping carrier dropdown with dynamic custom carrier input field, improved form state initialization to handle existing custom carriers, resolved order update functionality issues by correcting apiRequest parameter order and implementing proper admin session authentication with requireAdminAuth middleware and session cookie configuration
```

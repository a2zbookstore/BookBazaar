...
- June 18, 2025. Configured production SMTP with user's Zoho Mail credentials - updated email service to use orders@a2zbookshop.com via smtp.zoho.com:465 (SSL), implemented App Password authentication with graceful error handling, orders and status updates continue to work even if email fails, added comprehensive troubleshooting guidance in admin panel for SMTP configuration issues, system prioritizes order completion over email delivery.
```

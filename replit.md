...
- June 22, 2025. Implemented comprehensive forgot password functionality - added reset token fields to users database schema, created password reset API endpoints using Brevo SMTP email service, built forgot password dialog on login page with email input, created dedicated reset password page with secure token validation, integrated professional HTML email templates with A2Z BOOKSHOP branding, complete password reset workflow from request to email delivery to new password setup, using Brevo SMTP infrastructure for reliable delivery, fixed email configuration with proper sender address and both HTML/text formats for better deliverability.
```

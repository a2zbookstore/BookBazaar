...
- June 21, 2025. Created comprehensive "Gift with Purchase" promotional feature using Framer Motion animations - interactive gift selection carousel appears on homepage when items are added to cart, customers can select one free novel or notebook from 6 beautifully displayed options, selected gifts appear in cart and checkout with locked quantity of 1 and price of ₹0/FREE, complete integration with payment systems and order processing, smooth animations with selection highlighting and "Change Gift" functionality, localStorage-based gift persistence throughout shopping session.
